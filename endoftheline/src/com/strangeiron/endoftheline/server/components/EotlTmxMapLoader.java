...
public class EotlTmxMapLoader extends AsynchronousAssetLoader<TiledMap, EotlTmxMapLoader.Parameters> {
	 ...
	 * 
	 * @param resolver */
	public EotlTmxMapLoader (FileHandleResolver resolver) {
		super(resolver);
	}
	...
	public TiledMap load (String fileName) {
		return load(fileName, new EotlTmxMapLoader.Parameters());
	}
	 ...
	 * @return the TiledMap */
	public TiledMap load (String fileName, EotlTmxMapLoader.Parameters parameters) {
		try {
			...
			for (FileHandle textureFile : loadTilesets(root, tmxFile)) {
				...
				textures.put(textureFile.path(), texture); */ // --> Do not load textures!
			}
	...
	@Override
	public void loadAsync (AssetManager manager, String fileName, FileHandle file, EotlTmxMapLoader.Parameters parameter) {
		map = null;
	...
	@Override
	public TiledMap loadSync (AssetManager manager, String fileName, FileHandle file, EotlTmxMapLoader.Parameters parameter) {
		return map;
	}
	...
	@Override
	public Array<AssetDescriptor> getDependencies (String fileName, FileHandle file, EotlTmxMapLoader.Parameters parameter) {
						...
						for (int y = 0; y < height; y++) {
							for (int x = 0; x < width; x++) {
								...
								id = id & ~MASK_CLEAR;
                                                                ...
                                                                if(id > 0) 
                                                                {
                                                                    TiledMapTile tile = new EotlStaticTiledMapTile();
                                                                    Cell cell = createTileLayerCell(flipHorizontally, flipVertically, flipDiagonally);
                                                                    cell.setTile(tile);
                                                                    layer.setCell(x, yUp ? height - 1 - y : y, cell);
                                                                 }
